## P-value function for Wald statistic
##------------------------------------------------------------------------------
p_fun_wald <- function(h0_point, es, se, onesided = T){
  p <- if (onesided) {
    1 - pnorm(es, h0_point, se)
  } else if (!onesided) {
    2 * apply(cbind(pnorm(es, h0_point, se), 1 - pnorm(es, h0_point, se)), 1, min)
  }
  return(p)
}
...
## Plot simulation results: means
## -----------------------------------------------------------------------------
simplot <- function(dt, distr, stw_var, nam, hts = TRUE) {
  
  # filter not displayed methods
  ...
  dt |>
    ...
    mutate(
      ...
      I2 = factor(paste0("iota^2==", I2, "*'%'"),
                  levels = c("iota^2==0*'%'", "iota^2==30*'%'",
                             "iota^2==60*'%'", "iota^2==90*'%'")),
    ...
    summarise(
      me = mean(v, na.rm = T),
      se = sd(v, na.rm = T) / sqrt(sum(!is.na(v))),
      .groups = "drop"
    ) |>
    ...
    scale_fill_manual(values = cst, labels = lab) +
    theme_dk() +
    theme(
      axis.text = element_text(colour = "black", family = "Times", size = 8),
...
## Plot simulation results: pearson correlations
##------------------------------------------------------------------------------
plotcor <- function(dt, distr, stw_var, var, nam) {

  # filter displayed methods
  dt <- dt |> dplyr::select(-ci.sk.held.u, -pi.sk.hts, - ci.sk.r, -ci.sk.hk)
  ...
  dt |>
    ...
    geom_errorbar(aes(ymin = cr - se, ymax = cr + se),
                  ...
                  width = 0.75) +
    labs(x = "Number of studies") +
    scale_color_manual(values = cst, labels = lab, na.translate = FALSE) +
    scale_fill_manual(values = cst, labels = lab, na.translate = FALSE) +
    theme_dk() +
    theme(
      axis.text = element_text(colour = "black", family = "Times", size = 8),
      axis.title = element_text(colour = "black", size = 8),
...
## Plot simulation results: coverage probability distribution
## -----------------------------------------------------------------------------

pcvr <- function(i2f, distf) {
  
  lev <- c("hts", "nnf", "fix", "simple", "full")
  lab <- as.expression(c("HTS", "Bootstrap", "PCD-fixed", "PCD-simplified", "PCD-full"))
  cst <- c(hts = "#7C878EFF", nnf = "#84BD00FF", fix = "#FFCD00FF",
           simple = "#5C88DAFF", full = "#CC0C00FF")
  I2_levels <- c("iota^2==0*'%'", "iota^2==30*'%'", "iota^2==60*'%'", "iota^2==90*'%'")
  ...
  dtx |>
    ...
    mutate(
      ...
      I2 = factor("iota^2==0*'%'", levels = I2_levels),
...
## Plot simulation results: Cohens Kappa for sign agreement
## -----------------------------------------------------------------------------
